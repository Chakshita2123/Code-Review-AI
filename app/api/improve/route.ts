...
import { generateImprovedCode } from '@/lib/gemini';
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const approaches = await generateImprovedCode(code, language as SupportedLanguage);
    return NextResponse.json({ success: true, approaches });
  } catch (error) {
    console.error('Improved code generation failed:', error);
    return NextResponse.json(
      { success: false, message: 'Unable to generate improved code. Please try again.' },
      { status: 500 },
    );
  }
}
